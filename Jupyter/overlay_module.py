...
RUNTIME_BNN = "python_hw" 
RUNTIME_CNN = "python_sw" 
...
_ffi = cffi.FFI()
_ffi.cdef(""" void load_parameters(const char* path); int inference(const char* 
path, int results[64], int number_class, float *usecPerImage); int* inference_multiple(const char* path, int number_class, int 
*image_number, float *usecPerImage, int enable_detail); void free_results(int * result); void deinit(); """ ) 
...
class PynqBNN:
    # on creating PynqBNN the shared library for a given network is loaded and bitstream is downloaded to PL when intending to use 
    # hardware accelerated runtime
    def __init__(self, runtime, network, load_overlay=True):
        self.bitstream_name = None
        if runtime == RUNTIME_BNN:
    ...
    def inference(self, path):
        ...
        return result_ptr
    # starts inference on single image, output is vector containing rankings of all available classes not available for LFC
...
class CnvClassifier:
    ...
    def __init__(self, network, params, runtime=RUNTIME_BNN):
    ...
    def classify_cifars(self, path):
        ...
        self.usecPerImage = self.bnn.usecPerImage
        return result
    # multiple detailed inference returns a flatten 1 dimensional vector with each ranking for each class, image by image .. for 
    # regular images
